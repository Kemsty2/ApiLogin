...
passport.use('register', new LocalStrategy({
	...
	User.findOne({
		...
		if(user){
			return done(null, false, req.flash('registerMessage', {result: false, error: '11'}));
		}else{
...
passport.use('login', new LocalStrategy({
	...
	User.findOne({
		...
		if (!user) {
			return done(null, false, req.flash('loginMessage', {result: false, error: '01'}));
		}
		if (!isValidPassword(user.password, passport)) {
			return done(null, false, req.flash('loginMessage', {result: false, error: '02'}));
		}
		...
		console.log('Error', err);
		return done(null, false, req.flash('loginMessage', {result: false, error: '00'}));
	})
}));
